...
6. 输出正文时包含标题、统计周期、核心摘要、本周焦点、分类热点分析和总体判断；不要单独输出来源明细表。每条动态必须在正文中保留发布时间，并把来源名称写成可点击链接。
7. 按用户要求选择交付方式：默认直接在聊天中返回报告正文；用户要求输出 PDF 时，再按 `references/pdf-report-format.md` 整理结构化 JSON，并调用 `scripts/render_weekly_pdf.py` 生成 PDF 文件；用户要求输出 Markdown/md 文件时，写出 `.md` 文件并返回文件路径。

## 交付方式

默认交付：直接在聊天中返回中文《本周热点分析》正文，不生成 PDF，不生成 Markdown 文件。
...
如果当前运行环境没有 Python、没有文件系统、不能执行命令，或找不到带 `reportlab` 的 Python runtime，必须明确说明“当前环境不能直接生成 PDF 文件”，并输出报告正文作为降级结果；不要在线安装依赖、切换到 HTML/Word/LaTeX 或声称已生成 PDF。
...
```bash
python3 scripts/lint_weekly_report.py report.md
```

默认在聊天中直接返回报告正文时，也按同一检查项自检；发现缺少固定栏目、统计周期、来源链接、发布时间、AI 摘要或分类数量不合格时，只修正问题部分再交付。不要把候选统计、搜索入口统计或来源明细表作为默认交付内容。

用户要求 PDF 时，先确保同一报告正文满足 Markdown 检查项，再执行 `references/pdf-report-format.md` 中的 renderer 和文件存在检查。renderer 报告 JSON 字段缺失时，补齐报告数据后重跑，不要交付空壳 PDF。
...
Markdown 版式以结构清晰、来源链接完整、段落不臃肿为准。默认只在聊天中返回正文；用户要求 Markdown 文件时才写 `.md` 文件；用户要求 PDF 时才生成 PDF。PDF 是同一报告内容的格式化交付版本，栏目仍按实际领域和资讯分类生成，不写死业务栏目。
...
默认只输出中文《本周热点分析》正文，不附加执行说明、候选统计、搜索入口统计或来源明细表。
...
- 用户要求 PDF 时，没有真实 PDF 文件路径不得宣称 PDF 已生成。
- 用户要求 Markdown 文件时，必须写出 `.md` 文件并返回路径；默认聊天交付不需要写文件。
...
- `scripts/lint_weekly_report.py`：Markdown 报告交付前的轻量契约检查。
