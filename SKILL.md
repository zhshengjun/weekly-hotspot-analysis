...
1. 确认领域和统计周期。用户未指定领域时，追问用户明确重点，不要替用户预设领域。用户未指定周期时，默认使用当前日期往前 7 天；如用户说“本周”，按当前自然周解释，并在报告中写明日期范围。
2. 读取 `references/crawling-guide.md`，收集资讯。优先使用用户指定信源；可用搜索引擎和网页搜索广泛发现线索，不限于政府官网；最终保留官方、监管、协会、主流媒体、公司公告、研究机构、微信公众号等可追溯来源。
...
Markdown 版式以结构清晰、表格完整、段落不臃肿为准；不要承诺或生成 HTML、PDF、JSON 等额外格式，除非用户另行明确要求且当前客户端支持。
