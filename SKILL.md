...
4. 执行后检查 `output/pdf/report.pdf` 是否存在；能使用 `pdfplumber` 或 `pypdf` 时，抽取文本检查标题、分类、总体判断和至少一个来源名称。
5. 最终回复必须给出 PDF 文件路径，并简要说明已生成 PDF。不要把生成 PDF 的 Python 代码贴给用户后就收工。

如果当前运行环境没有 Python、没有文件系统、不能执行命令，或缺少 `reportlab` 且无法安装，必须明确说明“当前环境不能直接生成 PDF 文件”，并输出报告正文作为降级结果；不要声称已生成 PDF。
